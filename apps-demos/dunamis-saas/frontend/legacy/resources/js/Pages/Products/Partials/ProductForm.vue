...
const deshabilitarArs = computed(() => form.cost_usd !== null && form.cost_usd !== '' && Number(form.cost_usd) > 0)
const deshabilitarUsd = computed(() => form.cost_ars !== null && form.cost_ars !== '' && Number(form.cost_ars) > 0)
...
watch(
    () => form.cost_usd,
    (nuevo) => {
        if (bloqueaSync.value) return
        if (nuevo !== null && nuevo !== '' && tasaNumero.value) {
            bloqueaSync.value = true
            const calculado = tasaNumero.value > 0 ? Number(nuevo) * tasaNumero.value : form.cost_ars
            form.cost_ars = calculado !== null && calculado !== undefined ? Number(calculado.toFixed(2)) : form.cost_ars
            bloqueaSync.value = false
        }
    }
)
...
watch(
    ...
    (nuevo) => {
        ...
        if (nuevo !== null && nuevo !== '' && tasaNumero.value) {
            bloqueaSync.value = true
            const calculado = tasaNumero.value > 0 ? Number(nuevo) / tasaNumero.value : form.cost_usd
            form.cost_usd = calculado !== null && calculado !== undefined ? Number(calculado.toFixed(2)) : form.cost_usd
            bloqueaSync.value = false
        }
    }
)
